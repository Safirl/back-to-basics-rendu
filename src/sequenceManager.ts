...
export default class SequenceManager {
  ...
  startExperience() {
    ...
    const dialogueModal = new Dialogue(
      currentDialogue,
      window.innerWidth / 2,
  ...
  intro() {
    if (this.currentIntroDialogue == 0) {
      this.eventEmitter.emit("introStarted");
    }
    if (this.currentIntroDialogue > 0) {
  ...
  showScene() {
    this.eventEmitter.emit("sceneLoaded");
  ...
  setCursedScene() {
    this.eventEmitter.emit("onCursedScene");
